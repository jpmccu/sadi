...
class TestPython312CompatibilityStandalone(unittest.TestCase):
    ...
    def test_no_buffer_function_calls(self):
        ...
        for filepath in python_files:
            ...
            if any(skip in str_path for skip in ['test', '.eggs', 'ez_setup']):
                continue
        ...
        self.assertEqual(len(buffer_usages), 0, 
                        f"Found buffer() calls in: {buffer_usages}")
    ...
    def test_no_future_library_imports(self):
        ...
        self.assertEqual(len(future_imports), 0, 
                        f"Found deprecated future imports: {future_imports}")
    ...
    def test_no_deprecated_constructs_usage(self):
        ...
        self.assertEqual(len(deprecated_usages), 0, 
            ...
            try:
                ...
                for i, line in enumerate(lines, 1):
                    ...
                    if re.search(r'%[sd].*%.*\(', line):
                        potential_issues.append(f"{filepath}:{i}: Possible old-style string formatting")
            except (UnicodeDecodeError, OSError):
                continue
                ...
                try:
                    ...
                    for node in ast.walk(tree):
                        if hasattr(ast, 'Print') and isinstance(node, ast.Print):
                            print_issues.append(f"{filepath}:{node.lineno}: Print statement found")
                except SyntaxError:
        ...
        for filepath in python_files:
            try:
                ...
                try:
                    ...
                    has_encoding = (
                        'coding' in first_str or 'coding' in second_str or
                        'encoding' in first_str or 'encoding' in second_str
                    )
                    
                    if not has_encoding:
                        encoding_info.append(f"{filepath}: Contains non-ASCII but no encoding declaration")
            except (UnicodeDecodeError, OSError):
                continue
        
        # This is informational - modern Python handles UTF-8 by default
        if encoding_info:
            print(f"INFO: Files with non-ASCII characters but no encoding declaration: {encoding_info}")
...
class TestPackageStructure(unittest.TestCase):
    ...
    def test_setup_py_python_requires(self):
        ...
        if setup_py.exists():
            ...
            self.assertIn('python_requires', content, 
            ...
            if 'python_requires' in content:
                ...
                if match:
                    ...
                    self.assertTrue(
                        '>=3.8' in requirement or '>=3.9' in requirement or '>=3.10' in requirement,
    ...
    def test_package_imports(self):
        ...
        if sadi_dir.exists():
            ...
            self.assertTrue(init_file.exists(), 
                           "sadi package should have __init__.py")
