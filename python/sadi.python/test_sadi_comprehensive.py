                    ...
                    with open(filepath, 'r', encoding='utf-8') as f:
                        ...
                        for i, line in enumerate(lines, 1):
                            if line.strip().startswith('#'):
                                continue
                            if 'buffer(' in line:
                                self.fail(f"Found buffer() call in {filepath}:{i}: {line.strip()}")
                    ...
                    with open(filepath, 'r', encoding='utf-8') as f:
                        ...
                        for i, line in enumerate(lines, 1):
                            ...
                            if any(imp in line for imp in [
                                ...
                                self.fail(f"Found future import in {filepath}:{i}: {line.strip()}")
                    ...
                    with open(filepath, 'r', encoding='utf-8') as f:
                        ...
                        for i, line in enumerate(lines, 1):
                            ...
                            if 'basestring' in line:
                                self.fail(f"Found deprecated construct in {filepath}:{i}: {line.strip()}")
...
class TestFRIRModule(unittest.TestCase):
    ...
    def test_rdf_graph_digest(self):
        ...
        graph, item = digest.fstack(StringIO(self.test_rdf_turtle), mimetype="text/turtle")
    ...
    def test_graph_canonicalization(self):
        ...
        # Extract hash values
        digest1_resources = list(graph1[:RDF.type:URIRef("http://purl.org/twc/ontology/frir.owl#RDFGraphDigest")])
        digest2_resources = list(graph2[:RDF.type:URIRef("http://purl.org/twc/ontology/frir.owl#RDFGraphDigest")])
        ...
        hash1 = list(graph1[digest1_resources[0]:URIRef("http://www.semanticdesktop.org/ontologies/2007/03/22/nfo#hashValue"):])[0]
        hash2 = list(graph2[digest2_resources[0]:URIRef("http://www.semanticdesktop.org/ontologies/2007/03/22/nfo#hashValue"):])[0]
    ...
    def setUp(self):
        ...
        class AsyncTestService(sadi.Service):
            ...
            def async_process(self, input_resource, output_resource):
                ...
                name = input_resource.value(foaf.name)
                if name:
                    output_resource.set(hello.greeting, Literal(f"Async Hello, {name.value}"))
